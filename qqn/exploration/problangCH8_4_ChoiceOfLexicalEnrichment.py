...
def speaker_LU(belief_state, lexicon, alpha=1., num_samples=10):
    def model():
...
def listener_LU(utterance, num_samples=10):
    def model():
        ...
        speak = speaker_LU(belief_state, lexicon)
...
def speaker_LEC(belief_state, alpha=1., num_samples=10):
    def model():
        utterance = utterance_prior().sample()
        lexicon = lexicon_prior()
        util = utility(belief_state, utterance, lexicon)
        alpha_t = tensor(alpha).float()
        pyro.factor("speak_LEC_factor", alpha_t * util)
        return utterance
...
def listener_LEC(utterance):
    def model():
        knowledgeability = knowledgeability_level_prior()
        # felher auf der website ?: knowledgeability_level_prior(speaker_knowledgeability_states)
        lexicon = lexicon_prior()
        belief_state = belief_state_prior(knowledgeability)
        speak = speaker_LEC(belief_state)
        pyro.factor("speak_LEC_factor", speak.log_prob(torch.stack(utterance, lexicon)))
        return torch.stack(belief_state, lexicon)
