...
ds_trajectory = [(tensor([11, 3, 6]), None), (tensor([10, 2, 6]), tensor(3)), (tensor([9, 1, 6]), tensor(3))]
dn_trajectory = [(tensor([11, 3, 6]), None),
                 (tensor([10, 3, 5]), tensor(0)),
                 (tensor([9, 3, 4]), tensor(0)),
                 (tensor([8, 3, 3]), tensor(0))]
noodle_trajectory = [(tensor([11, 3, 6]), None),
                     (tensor([10, 3, 5]), tensor(0)),
                     (tensor([9, 3, 4]), tensor(0)),
                     (tensor([8, 4, 4]), tensor(1)),
                     (tensor([7, 5, 4]), tensor(1))]
veg_trajectory = [(tensor([11, 3, 6]), None),
                  (tensor([10, 3, 5]), tensor(0)),
                  (tensor([9, 3, 4]), tensor(0)),
                  (tensor([8, 3, 3]), tensor(0)),
                  (tensor([7, 3, 2]), tensor(0)),
                  (tensor([6, 3, 1]), tensor(0)),
                  (tensor([5, 2, 1]), tensor(1)),
                  ]
n_trajectory = [(tensor([11, 3, 6]), None),
                (tensor([10, 3, 5]), tensor(0))]

don_or_veg_trajectory = [(tensor([11, 3, 6]), None),
                         (tensor([10, 3, 5]), tensor(0)),
                         (tensor([9, 3, 4]), tensor(0)),
                         (tensor([8, 3, 3]), tensor(0)),
                         (tensor([7, 3, 2]), tensor(0)),
...
outer_trace = don_or_veg_trajectory
...
def outer_state_value_function(state):
    with block():
        with (inner_nr_of_actions,
            ...
            traces = [simulate_eff(gen_inner_initial_state(state)) for _ in trange(250)]
    matching_traces = 0
    for trace in traces:
        pointer = 0
        while pointer < len(outer_trace):
            outer_state, outer_last_action = outer_trace[pointer]
            inner_state, inner_last_action = trace[pointer]
            if outer_last_action != inner_last_action:
                break
            pointer += 1
        if pointer == len(outer_trace):
            matching_traces += 1
    print()
    print("Matching traces:", matching_traces)
    print()
    return tensor(matching_traces)
...
with (outer_nr_of_actions,
      ...
      outer_state_isfinal,
      softmax_agent(),
      sampling_estimating_agent(optimization_steps=10, max_estimation_value=100),
      ):
